...
@testset "pcf_serial" begin
    ...
    @test output == (
        rs=[1.0, 2.0],
        pcf=[1.185825438681694, 0.9491433166161567],
        stderr=[5.18352815817258, 3.1353830626716523],
        n_MC=100000,
    )
    ...
    @test output == (
        rs=[1.0, 2.0],
        pcf=[0.004403745642155812, 0.018468257258804625],
        stderr=[0.21541086193483894, 0.7090098848892278],
        n_MC=100000,
    )
end
...
@testset "pcf_threads" begin
    ...
    @test isapprox(output.stderr, [5.50894290323668, 3.1070562772609533], rtol=1e-2)
    ...
    @test isapprox(output.pcf, [0.004667458716599647, 0.019432243383040823], rtol=1e-2)
    @test isapprox(output.stderr, [0.2548776852703169, 0.8368493788235667], rtol=1e-2)
end
